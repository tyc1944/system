    ...
    <table class="layui-table">
        ...
        <tr>
            ...
            <td>
                任务状态:
                <!-- 跟踪任务展示块-->
                <select id="type" name="taskstate" class="valid" >
                    <option value="未实施" <c:if test="${Task.taskstate.equals('未实施')}">selected</c:if>>
                        未实施
                    </option>
                    <option value="实施中" <c:if test="${Task.taskstate.equals('实施中')}">selected</c:if>>
                        实施中
                    </option>
                    <option value="已完成" <c:if test="${Task.taskstate.equals('已完成')}">selected</c:if>>
                        已完成
                    </option>
                </select>
...
<script>
    $(function (){
            var url ="${pageContext.request.contextPath}/taskdetailstrack";

            $("#taskstate").change(function(){
                var state = $('#taskstate option:selected').val();
                $("#state").val(state);
                $.ajax({
                    url:url,                 //必须采用同步方法
                    data: {"taskstate": $("#state").val(),"tid":$("#tid").val()},
                    type: 'POST',
                    // async:false
