...
<%--            <td class="td-manage">--%>
<%--                <a title="制定计划"--%>
<%--                   onclick="x_admin_show('制定计划','task-details.html',700,450)"--%>
<%--                   href="javascript:;"> <i class="layui-icon">&#xe60a;</i> </a>--%>
<%--            </td>--%>
        </tr>
    </c:forEach>
    ...
    <div class="page">
        <div>
            ...
            <a class="prev" href="${pageContext.request.contextPath}/tasks-list?page=${pageInfo.pageNum-1}">&lt;&lt;</a>
            <c:forEach var="p" begin="1" end="${pageInfo.pages}">
                <c:if test="${p!=pageInfo.pageNum}">
                    <a class="num" href="${pageContext.request.contextPath}/task-list?page=${p}">${p}</a>
                </c:if>
                <c:if test="${p==pageInfo.pageNum}">
                    <span class="current">${p}</span>&nbsp;
                </c:if>
            </c:forEach>
            <a class="next" href="${pageContext.request.contextPath}/task-list?page=${pageInfo.pageNum+1}">&gt;&gt;</a>
        </div>
    </div>
